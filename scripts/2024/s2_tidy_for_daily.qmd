...
```{r, eval=FALSE}
# lst = foreach(f = fs, i = icount(10)) %do% {
#   runningId(i)
#   d <- fread(f) |> mutate(date = date + dhours(8))
#   info <- d[, .(date, Rs)] %>%
...
lst_day = foreach(f = fs, site = names(fs), i = icount()) %do% {
  runningId(i)
...
```{r}
r <- map(lst_day, names)
write_json(r, "variinfo_fullset_st212_day.json", pretty = TRUE)
```

```{r}
# df_out <- map(lst_day, \(d) select(d, -starts_with(c("TS_F_", "SWC_F_")))) |>
#   melt_list("site")
version <- glue("v20260331 ({minValidPerc*100}%)")

df_out <- rbindlist(lst_day, fill = TRUE)
